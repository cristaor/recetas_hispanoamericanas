...
class Vista_lista_apuestas(QWidget):
    ...
    def mostrar_apuestas(self, nombre_carrera,  apuestas):
        ...
        if len(self.apuestas) == 0:
            self.etiqueta_nombre.setText('Apuestas para {}'.format(nombre_carrera) + " (sin Apuestas)")


        numero_fila=1
        for apuesta2 in self.apuestas:
            
            if type(apuesta2) is dict:
                apuesta=apuesta2
            else:
                apuesta=dict(apuesta2)

            apostador2=self.interfaz.dar_apostador(apuesta["apostador"])
            print("Nombre de apostador " + apostador2[0].nombre)
            apuesta["apostador"]=apostador2[0].nombre;

            competidor2=self.interfaz.dar_competidor(apuesta["competidor"])
            print("Nombre de competidor " + competidor2[0].nombre)
            apuesta["competidor"]=competidor2[0].nombre;
            #apuesta["valor"]=float(apuesta["valor"])
            ...
            etiqueta_valor = QLabel("$" + apuesta["valor"])
